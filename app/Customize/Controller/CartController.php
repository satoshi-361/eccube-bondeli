...
class CartController extends AbstractController
{
        ...
        foreach ($Carts as $Cart) {
            ...
            // if ($store_id == 0) {
                foreach($Cart->getCartItems() as $CartItem) {
                    $lower_price_limit = $CartItem->getProductClass()->getProduct()->getRestaurant()->getLowerPriceLimit();
                    $request->getSession()->set('lower_price_limit', $lower_price_limit);
                    break;
                }
            // }
            ...
            if ($this->baseInfo->getDeliveryFreeQuantity()) {
                if (
                    $this->baseInfo->getDeliveryFreeQuantity() >
                    $Cart->getQuantity()
                ) {
            ...
            if ($this->baseInfo->getDeliveryFreeAmount()) {
                if (
                    ...
                    $this->baseInfo->getDeliveryFreeAmount() <=
                        $Cart->getTotalPrice()
                ) {
                    $isDeliveryFree[$Cart->getCartKey()] = true;
                } else {
    ...
    public function handleCartItem($operation, $productClassId)
    {
        ...
        if (is_null($ProductClass)) {
            log_info('商品が存在しないため、カート画面へredirect', [
                'operation' => $operation,
                'product_class_id' => $productClassId,
            ]);
    ...
    public function buystep(Request $request, $cart_key)
    {
        ...
        $event = new EventArgs([], $request);
        $this->eventDispatcher->dispatch(
            EccubeEvents::FRONT_CART_BUYSTEP_INITIALIZE,
            $event
        );
        ...
        $event = new EventArgs([], $request);
        $this->eventDispatcher->dispatch(
            EccubeEvents::FRONT_CART_BUYSTEP_COMPLETE,
            $event
        );
