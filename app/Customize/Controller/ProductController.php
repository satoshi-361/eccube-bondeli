    ...
    public function addCart(Request $request, Product $Product)
    {
        ...
        foreach ($Carts as $Cart) {
            foreach($Cart->getCartItems() as $item) {
                $Restaurants[] = $item->getProductClass()->getProduct()->getRestaurant()->getId();
            }
        }
        array_unique($Restaurants, SORT_REGULAR);
        if (!in_array($Product->getRestaurant()->getId(), $Restaurants) && count($Restaurants) > 0)
            return $this->json(['done' => false, 'messages' => ['他の加盟店の料理はショッピングカートに入れることはできません。']]);
