...
class ShoppingController extends AbstractShoppingController
{
    ...
    public function index(PurchaseFlow $cartPurchaseFlow, Request $request)
    {
        ...
        $url .= $Customer->getPostalCode();
        $data   = @file_get_contents($url);
        $result = json_decode($data, true);

        $customer_city = $result['results'][0]['address2'];
        $Restaurant = null;

        foreach($Cart->getCartItems() as $CartItem) {
            $Restaurant = $CartItem->getProductClass()->getProduct()->getRestaurant();
            break;
        }

        $deliverable_cities = $Restaurant->getDeliverableArea();
        
        if (strpos($deliverable_cities, $customer_city) === false) {
            // return $this->redirectToRoute('homepage');
            return $this->redirectToRoute('shopping_error', ['message'=> 'こちらのレストランは、お客様の住所への配達地域外となっております。; 申し訳ありませんが、今回の注文はお受けできません。']);
        }
        ...
        $lower_price_limit = 5000;
        if (null !== $request->getSession()->get('lower_price_limit')) {
            $lower_price_limit = $request->getSession()->get('lower_price_limit');
        }
    ...
    public function checkout(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $accept_url = $this->encryptUrl($Order->getId().'_accept');
            $reject_url = $this->encryptUrl($Order->getId().'_reject');
            
            $Order->setAcceptUrl('https://bon-deli.com/order/manage/'.$accept_url);
            $Order->setRejectUrl('https://bon-deli.com/order/manage/'.$reject_url);
     ...
     * 購入エラー画面.
     *
     * @Route("/shopping/error/{message}", name="shopping_error")
     * @Template("Shopping/shopping_error.twig")
     */
    public function error(Request $request, PurchaseFlow $cartPurchaseFlow, $message = null)
    {
    ...
    public function process_card(Request $request) {
        ...
        $Order->setSqPaymentId($result['payment']['id']);
        $this->entityManager->persist($Order);
        $this->entityManager->flush();
    ...
    /**
     * @Route("/order/manage/{url}", name="order_manage")
     */
    public function orderManage($url){
        if(!$url){return false;}

        $url = $this->decryptUrl($url);

        if (strpos($url, '_accept') !== false) {
          $order_id = substr($url, 0, strpos($url, '_accept'));
          $Order = $this->orderRepository->find($order_id);
    ...
    public function orderReject(){
      ...
      $amount_money->setAmount(intval($Order->getTotalPrice()));
      $amount_money->setCurrency('JPY');

      $app_fee_money = new \SquareConnect\Model\Money();
      $app_fee_money->setAmount(intval($Order->getTotalPrice() * 0.03));
      $app_fee_money->setCurrency('JPY');

      $request_body = array(
        "idempotency_key" => uniqid(),
        "amount_money" => $amount_money,
        "payment_id" => $Order->getSqPaymentId(),
        "reason" => $message
      );

      $api_request = new \SquareConnect\Model\RefundPaymentRequest($request_body);
      ...
      try {
        $api_response = $refunds_api->refundPayment($request_body);

        $Order->setRejectMessage($message);
        $Order->setOrderStatus($this->getDoctrine()->getRepository(OrderStatus::class)->find(OrderStatus::RETURNED));
        $this->entityManager->persist($Order);
        $this->entityManager->flush();

        $this->mailService->sendRejectMail($Order);
    ...
    function decryptUrl($string, $key = 'secure_key') {
        $result = '';
      ...
      function encryptUrl($string, $key = 'secure_key') {
        $result = '';
        for($i=0; $i<strlen($string); $i++) {
          $char = substr($string, $i, 1);
          $keychar = substr($key, ($i % strlen($key))-1, 1);
