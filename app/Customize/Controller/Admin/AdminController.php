...
class AdminController extends AbstractController
{
    ...
    public function index(Request $request)
    {
        ...
        $Restaurants = $this->getDoctrine()->getRepository(Restaurant::class)->findBy(['state' => 3]);
        $resIds = [];

        foreach($Restaurants as $Restaurant) {
          $resIds[$Restaurant->getId()] = $Restaurant->getCompanyName();
        }
    ...
    public function sale(Request $request)
    {
        ...
        if ( isset($_POST['resId']) ) $resId = $_POST['resId'];
        else $resId = 0;

        // 週間の売上金額
        $toDate = Carbon::now();
        $fromDate = Carbon::today()->subWeek();
        $rawWeekly = $this->getData($fromDate, $toDate, 'Y/m/d', $resId);

        // 月間の売上金額
        $fromDate = Carbon::now()->startOfMonth();
        $rawMonthly = $this->getData($fromDate, $toDate, 'Y/m/d', $resId);

        // 年間の売上金額
        $fromDate = Carbon::now()->subYear()->startOfMonth();
        $rawYear = $this->getData($fromDate, $toDate, 'Y/m', $resId);
     ...
     * @return array
     */
    protected function getData(Carbon $fromDate, Carbon $toDate, $format, $resId)
    {
        ...
        if ($resId == 0) return $this->convert($result, $fromDate, $toDate, $format);
        else {
          $Orders = $result;
          $unset_keys = array();
          ...
          foreach($Orders as $key => $Order) {
              foreach($Order->getOrderItems() as $OrderItem) {
                  ...
                  if ($OrderItem->getProduct()->getRestaurant()->getId() != $resId) {
                      $Order->removeOrderItem($OrderItem);
                  }
              }
