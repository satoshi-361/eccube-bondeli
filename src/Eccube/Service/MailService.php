...
class MailService
{
	...
	/**
     * Send restaurant application mail.
     *
     * @param $formData 加盟レストラン申請内容
     */
	public function sendRestaurantApplyMail($formData)
	{
     ...
     * @return \Swift_Message
     */
    public function sendRejectMail(\Eccube\Entity\Order $Order)
    {
        log_info('返金メール送信開始');
    ...
    /**
     * Send order accept mail.
     *
     * @param \Eccube\Entity\Order $Order 受注情報
     *
     * @return \Swift_Message
     */
    public function sendAcceptMail(\Eccube\Entity\Order $Order)
    {
        log_info('返金メール送信開始');

        $MailTemplate = $this->mailTemplateRepository->find(12);
        $Restaurant = $email = $Order->getProductOrderItems()[0]->getProduct()->getRestaurant();
    ...
    /**
     * Send order2restaurant mail.
     *
     * @param \Eccube\Entity\Order $Order 受注情報
     *
     * @return \Swift_Message
     */
    public function sendOrder2Restaurant(\Eccube\Entity\Order $Order)
    {
        log_info('受注メール送信開始');
        ...
        $email = $Order->getProductOrderItems()[0]->getProduct()->getRestaurant()->getEmail();
    ...
    /**
     * Send order error mail.
     *
     * @param \Eccube\Entity\Order $Order 受注情報
     *
     * @return \Swift_Message
     */
    public function sendErrorMail(\Eccube\Entity\Order $Order)
    {
        log_info('返金メール送信開始');

        $MailTemplate = $this->mailTemplateRepository->find(13);
        $Restaurant = $email = $Order->getProductOrderItems()[0]->getProduct()->getRestaurant();
